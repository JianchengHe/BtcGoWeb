...
func (bc BlockChain) QueryBlockByCertId(cert_id string) (*Block, error) {
	...
	db.View(func(tx *bolt.Tx) error {
		...
		for {//无限循环遍历，查询对应cert_id的区块数据
			...
			if string(record.CertID) == cert_id { //说明找到了区块
				block = eachBlock
				break
			}
			fmt.Println("存到区块的数据：", string(eachBlock.Data))
			fmt.Println("cert_id:", cert_id)
			//把每个区块的PerviousHash变成大整数
			eachBig.SetBytes(eachBlock.PerviousHash)
			if eachBig.Cmp(zeroBig) == 0 { //到创世区块没有找到
				break
			}
			eachHash = eachBlock.PerviousHash
		}
